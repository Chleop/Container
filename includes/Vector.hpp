...
/*   Updated: 2022/07/06 16:23:55 by cproesch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// Default
    ...
    _alloc(a), _size(0), _capacity(0), _array(_alloc.allocate(_capacity)){}
// Fill
...
// Copy
    ...
    _alloc(x._alloc), _size(0), _capacity(0), _array(_alloc.allocate(_capacity))
    {
        *this = x;
    }
// Destructor
...
// OPERATEUR ET FONCTIONS D'ASSIGNATION
    ...
    {
// ------------->ici je n'ai pas mis if (_capacity > 0) parce que sinon tout n'est pas freed.
...
// MODIFIERS
    ...
    void            clear(void){for (iterator it = this->begin(); it < this->end(); it++)
                    {_alloc.destroy(&(*it));}}
